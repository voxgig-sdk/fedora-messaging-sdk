# FedoraMessaging SDK Search entity
...
class SearchEntity
  ...
  def data_set(args)
    if args
      @_data = FedoraMessagingHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FedoraMessagingHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Search>, Array] the matching Search items; raises FedoraMessagingError on failure
