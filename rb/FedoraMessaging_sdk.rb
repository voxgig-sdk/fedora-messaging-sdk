...
require_relative 'FedoraMessaging_types'


class FedoraMessagingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FedoraMessagingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FedoraMessagingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FedoraMessagingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FedoraMessagingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FedoraMessagingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FedoraMessagingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FedoraMessagingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FedoraMessagingError.new(
        "#{op}_allow",
        "FedoraMessagingSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FedoraMessagingError => err
    ...
    if fetched.is_a?(Hash)
      status = FedoraMessagingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FedoraMessagingError.new(
        "graphql_error", "FedoraMessagingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FedoraMessagingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
