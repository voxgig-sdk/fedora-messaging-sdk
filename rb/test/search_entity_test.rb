...
class SearchEntityTest < Minitest::Test
  def test_create_instance
    testsdk = FedoraMessagingSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = FedoraMessagingSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FEDORA_MESSAGING_TEST_SEARCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def search_basic_setup(extra)
  ...
  entid_env_raw = ENV["FEDORA_MESSAGING_TEST_SEARCH_ENTID"]
  ...
  if env["FEDORA_MESSAGING_TEST_LIVE"] == "TRUE"
    ...
    client = FedoraMessagingSDK.new(Helpers.to_map(merged_opts))
  end
