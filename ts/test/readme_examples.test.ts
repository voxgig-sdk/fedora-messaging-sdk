...
//     (\`new FedoraMessagingSDK(...)\` / \`FedoraMessagingSDK.test(...)\` become
//     \`FedoraMessagingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FedoraMessagingSDK'
...
// rewrite real client construction (\`new FedoraMessagingSDK(...)\` and any
// \`FedoraMessagingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FedoraMessagingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FedoraMessagingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
