...
module FedoraMessagingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FedoraMessagingResponse.new({})
    ...
    if fetch_err
      ...
      response = FedoraMessagingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FedoraMessagingResponse.new(fetched)
    else
