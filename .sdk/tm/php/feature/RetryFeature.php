...
// FedoraMessaging SDK retry feature
...
class FedoraMessagingRetryFeature extends FedoraMessagingBaseFeature
{
    ...
    public function init(FedoraMessagingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FedoraMessagingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FedoraMessagingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FedoraMessagingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
