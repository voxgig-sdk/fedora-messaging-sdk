...
// FedoraMessaging SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class FedoraMessagingTestFeature extends FedoraMessagingBaseFeature
{
    ...
    public function init(FedoraMessagingContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (FedoraMessagingContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = FedoraMessagingParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (FedoraMessagingContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(FedoraMessagingContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = FedoraMessagingParam::call($ctx, $k);
