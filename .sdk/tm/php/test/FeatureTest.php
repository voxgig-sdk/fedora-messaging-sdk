...
// FedoraMessaging SDK feature test
//
...
class FtCtrl extends FedoraMessagingControl
{
...
class FtClient
{
    ...
    private FedoraMessagingUtility $utility;

    public function __construct(FedoraMessagingUtility $utility, array $options)
    {
    ...
    public function get_utility(): FedoraMessagingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FedoraMessagingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FedoraMessagingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FedoraMessagingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FedoraMessagingConfig::make_feature($name);
    ...
    private static function buildUrl(FedoraMessagingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FedoraMessagingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FedoraMessagingResponse($fetched);
        }
    ...
    private function fail(FedoraMessagingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FedoraMessagingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
