...
import (
	...
	omni "github.com/voxgig-sdk/fedora-messaging-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FedoraMessagingError); is {
				out := map[string]any{
					"name":    "FedoraMessagingError",
...
func omniCtx(arg any, client *sdk.FedoraMessagingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FedoraMessagingSDK); is {
			client = live
