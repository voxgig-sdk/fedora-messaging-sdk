...
class SearchEntity
{
    ...
    {
        if ($args) {
            $this->_data = FedoraMessagingHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = FedoraMessagingHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws FedoraMessagingError on failure (item-5 convention).
     */
