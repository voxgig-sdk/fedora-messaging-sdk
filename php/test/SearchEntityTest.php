...
class SearchEntityTest extends TestCase
{
    ...
    {
        $testsdk = FedoraMessagingSDK::test(null, null);
    ...
    public function test_stream(): void
    {
        ...
        $base = FedoraMessagingSDK::test($seed, null);
        $seen = iterator_to_array($base->Search(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = FedoraMessagingConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = FedoraMessagingSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FEDORA_MESSAGING_TEST_SEARCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function search_basic_setup($extra)
{
    ...
    $client = FedoraMessagingSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("FEDORA_MESSAGING_TEST_SEARCH_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["FEDORA_MESSAGING_TEST_SEARCH_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["FEDORA_MESSAGING_TEST_LIVE"] === "TRUE") {
        ...
        $client = new FedoraMessagingSDK(Helpers::to_map($merged_opts) ?? []);
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["FEDORA_MESSAGING_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
