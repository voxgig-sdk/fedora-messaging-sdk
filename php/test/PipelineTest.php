...
// FedoraMessaging SDK pipeline test
//
...
{
    private static function utility(): FedoraMessagingUtility
    {
        return new FedoraMessagingUtility();
    }
    ...
    private static function ctx(array $over = []): FedoraMessagingContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FedoraMessagingContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FedoraMessagingOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FedoraMessagingError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FedoraMessagingOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FedoraMessagingMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FedoraMessagingOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FedoraMessagingMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FedoraMessagingOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FedoraMessagingMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FedoraMessagingSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FedoraMessagingMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FedoraMessagingResponse([]),
            'result' => new FedoraMessagingResult([]),
        ]);
        [, $err] = FedoraMessagingMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FedoraMessagingSpec([]),
            'response' => null,
            'result' => new FedoraMessagingResult([]),
        ]);
        [, $err] = FedoraMessagingMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FedoraMessagingSpec(['step' => 's']),
            'response' => new FedoraMessagingResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FedoraMessagingResult([]),
        ]);
        [, $err] = FedoraMessagingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FedoraMessagingSpec(['step' => 's']),
            'response' => new FedoraMessagingResponse(self::resp(200, ['v' => 1])),
            'result' => new FedoraMessagingResult([]),
        ]);
        [, $err] = FedoraMessagingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FedoraMessagingSpec(['step' => 's']),
            'response' => new FedoraMessagingResponse(self::resp(200, ['v' => 2])),
            'result' => new FedoraMessagingResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FedoraMessagingMakeResponse::call($ctx);
    ...
    {
        $preset = new FedoraMessagingResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FedoraMessagingSpec([]),
            'response' => new FedoraMessagingResponse([]),
            'result' => new FedoraMessagingResult([]),
        ...
        [$got, $err] = FedoraMessagingMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FedoraMessagingResult([])]);
        [, $err] = FedoraMessagingMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FedoraMessagingSpec([]), 'result' => null]);
        [, $err] = FedoraMessagingMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FedoraMessagingSpec(['step' => 's']),
            'result' => new FedoraMessagingResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FedoraMessagingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FedoraMessagingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FedoraMessagingSpec(['step' => 's']),
            'result' => new FedoraMessagingResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FedoraMessagingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FedoraMessagingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FedoraMessagingResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FedoraMessagingSpec([]),
            'result' => new FedoraMessagingResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FedoraMessagingMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FedoraMessagingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FedoraMessagingError('boom', 'boom');
        $utility->fetcher = function (FedoraMessagingContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FedoraMessagingSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FedoraMessagingMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FedoraMessagingContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FedoraMessagingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FedoraMessagingResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        FedoraMessagingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FedoraMessagingContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FedoraMessagingSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FedoraMessagingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FedoraMessagingResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FedoraMessagingSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FedoraMessagingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FedoraMessagingMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FedoraMessagingMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FedoraMessagingResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FedoraMessagingDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FedoraMessagingResult(['ok' => false])]);
        $this->expectException(FedoraMessagingError::class);
        FedoraMessagingDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FedoraMessagingResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FedoraMessagingMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FedoraMessagingMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FedoraMessagingBaseFeature();
        $b = new FedoraMessagingBaseFeature();
        FedoraMessagingFeatureAdd::call($ctx, $a);
        FedoraMessagingFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FedoraMessagingBaseFeature
    {
        $f = new FedoraMessagingBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FedoraMessagingFeatureAdd::call($ctx, self::named_feature('a'));
        FedoraMessagingFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        FedoraMessagingFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        FedoraMessagingFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        FedoraMessagingFeatureAdd::call($ctx, $replace);
        ...
        FedoraMessagingFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?FedoraMessagingSpec $spec): FedoraMessagingContext
    {
    ...
    private static function auth_bags(): FedoraMessagingSpec
    {
        return new FedoraMessagingSpec(['headers' => [], 'query' => []]);
    }
    ...
    private static function auth_probe(array $options): ?array
    {
        ...
        FedoraMessagingPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FedoraMessagingPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FedoraMessagingResponse(['headers' => null]),
            'result' => new FedoraMessagingResult([]),
        ]);
        FedoraMessagingResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        FedoraMessagingResultBody::call($ctx);
