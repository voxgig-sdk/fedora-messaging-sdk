# fedora-messaging-mcp

[MCP](https://modelcontextprotocol.io) server exposing the FedoraMessaging SDK as
two agent tools — `fedora-messaging_list` and `fedora-messaging_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fedora-messaging-mcp)
make build

# 2. Provide credentials via the environment
export FEDORA_MESSAGING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user fedora-messaging \
  -- /absolute/path/to/fedora-messaging-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fedora-messaging-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fedora-messaging_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// fedora-messaging_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fedora-messaging-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fedora-messaging \
     -- "$PWD"/dist/*/fedora-messaging-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fedora-messaging_list` and `fedora-messaging_load` tools now appear
   in new sessions. Ask the agent to *"list search using fedora-messaging"*
   and it calls `fedora-messaging_list` with `{"entity":"search"}`.
...
```sh
export FEDORA_MESSAGING_APIKEY=sk_live_xxx            # API key
export FEDORA_MESSAGING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `fedora-messaging_list` tool
...
### Call the `fedora-messaging_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `fedora-messaging_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fedora-messaging_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FEDORA_MESSAGING_APIKEY` | API key sent with every request. |
| `FEDORA_MESSAGING_BASE` | Optional override of the API base URL. |
...
```sh
./fedora-messaging-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fedora-messaging_load","arguments":{"entity":"search","query":{"id":1}}}}'
```
