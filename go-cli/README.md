...
boru-driven command-line client **and** interactive REPL for the FedoraMessaging
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fedora-messaging-cli)
make build

# 2. See usage (words, entities, env vars)
./fedora-messaging-cli --help

# 3. Provide credentials once, via the environment
export FEDORA_MESSAGING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./fedora-messaging-cli list search

# 5. Override the API base URL for a single call
FEDORA_MESSAGING_BASE=https://api.example.com ./fedora-messaging-cli list search

# 6. No arguments -> interactive REPL
./fedora-messaging-cli
fedora-messaging> list search
fedora-messaging> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fedora-messaging-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/fedora-messaging-cli list search
   ```
...
Run with no arguments to open a REPL (prompt `fedora-messaging>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fedora-messaging-cli`. |
...
(`./fedora-messaging-cli <expr>`) and interactively (the REPL), and expressions compose the
