# FedoraMessaging SDK test runner
...
class FedoraMessagingTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FedoraMessagingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FedoraMessagingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FedoraMessagingTestRunner.getenv("FEDORA_MESSAGING_TEST_LIVE")
        override = FedoraMessagingTestRunner.getenv("FEDORA_MESSAGING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FedoraMessagingTestRunner.getenv(key)
        ...
        explain = FedoraMessagingTestRunner.getenv("FEDORA_MESSAGING_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FEDORA_MESSAGING_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FedoraMessagingTestRunner._test_control is not None:
            return FedoraMessagingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FedoraMessagingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FedoraMessagingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FedoraMessagingTestRunner.load_test_control()
...
def load_env_local():
    FedoraMessagingTestRunner.load_env_local()


def env_override(m):
    return FedoraMessagingTestRunner.env_override(m)


def entity_data(v):
    return FedoraMessagingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FedoraMessagingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FedoraMessagingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FedoraMessagingTestRunner.load_test_control()


def live_client_options():
    return FedoraMessagingTestRunner.live_client_options()


def live_delay_ms():
    return FedoraMessagingTestRunner.live_delay_ms()
