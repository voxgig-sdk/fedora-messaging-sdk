...
class TestSearchEntity:
    ...
    def test_should_stream(self):
        ...
        base = FedoraMessagingSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FEDORA_MESSAGING_TEST_SEARCH_ENTID JSON to run live")
        client = setup["client"]
...
def _search_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FEDORA_MESSAGING_TEST_SEARCH_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FEDORA_MESSAGING_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = FedoraMessagingSDK(helpers.to_map(merged_opts))

    _live = env.get("FEDORA_MESSAGING_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FEDORA_MESSAGING_TEST_EXPLAIN") == "TRUE",
        "live": _live,
