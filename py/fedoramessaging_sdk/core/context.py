...
from fedoramessaging_sdk.core.helpers import get_ctx_prop, to_map
...
class FedoraMessagingContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = FedoraMessagingControl()
        ...
        if isinstance(sp, FedoraMessagingSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, FedoraMessagingResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, FedoraMessagingResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return FedoraMessagingOperation({})
    ...
    def make_error(self, code, msg):
        return FedoraMessagingError(code, msg, self)
